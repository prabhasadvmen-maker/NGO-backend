...
// POST /api/superadmin/communication/send
export const sendBulkCommunication = async (req, res) => {
  try {
    const { type, subject, message, recipientType, branch } = req.body;
    ...
    return res.status(500).json({ success: false, message: 'Failed to send bulk communication' });
  }
};

// GET /api/superadmin/communication/logs
export const getCommunicationLogs = async (req, res) => {
  try {
    const { page = 1, limit = 10, type = '', branch = '' } = req.query;
...
// DELETE /api/superadmin/communication/logs/:id
export const deleteCommunicationLog = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await CommunicationLog.findByIdAndDelete(id);
    if (!deleted) {
...
// GET /api/superadmin/communication/mock-inbox
export const getMockCommunicationInbox = async (req, res) => {
  try {
    let mockInbox = [];
    ...
    return res.status(200).json({ success: true, data: mockInbox });
  } catch (error) {
    console.error('Get mock inbox error:', error);
    return res.status(500).json({ success: false, message: 'Failed to read mock mailbox' });
  }
};
